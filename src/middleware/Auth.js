...
const IsAuthenticateUser = TryCatch(async (req, res, next) => {
  ...
  try {
    ...
    const decodeData = jwt.verify(bearerToken, "sachinpitamaasweb");
...
const IsAuthenticateInfulncer = TryCatch(async (req, res, next) => {
  const bearerHeader = req.headers["authorization"];
  if (!bearerHeader) {
    return res.status(403).json({ error: "Unauthorized access" });
  }
  ...
  try {
    ...
    req.user = await Inful.findById(decodeData.id);
    next();
  } catch (error) { 
    if (error instanceof jwt.JsonWebTokenError) {
      res.status(400).json({ error: 'Invalid Token.' });
...
module.exports = { IsAuthenticateUser, authorizeRole,IsAuthenticateInfulncer };
